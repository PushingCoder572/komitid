...
def get_google_api_link():
    ...
    flow = Flow.from_client_secrets_file("resources/client_secret.json", scopes=scopes, redirect_uri='urn:ietf:wg:oauth:2.0:oob')
...
def get_tomorrows_morning(credentials):
    ...
    min_time = datetime.combine(tomorrow, start_time).strftime("%Y-%m-%dT%H:%M:%SZ")

    stop_time = datetime.strptime('23:00', '%H:%M').time()
    max_time = datetime.combine(tomorrow, stop_time).strftime("%Y-%m-%dT%H:%M:%SZ")
...
class Trip:
    ...
    def get_leg_info(self):
        ...
        for i in self.info["LegList"]["Leg"]:
            if i["type"] != "WALK":
                line_class = i["Product"]["name"].replace('tunnelbanans ', '').replace(' linje', 'linje').split(' ')
                final.append({
...
def sl_search(start, dest):
    ...
    for i, search_string in enumerate(station_info):
        get_site_id = 'https://api.sl.se/api2/typeahead.json?key=' + search_key + '&searchstring=' + search_string + '&stationsonly=False&maxresults=1'
...
def sl_get_trip(origin, dest, dest_time, dest_date_offset=1):
    ...
    if type(origin_in) == tuple:
        origin_string = "originCoordLat=" + origin_in[0] + "&originCoordLong=" + origin_in[1]
    else:
    ...
    if type(dest_in) == tuple:
        dest_string = "destCoordLat=" + dest_in[0] + "&destCoordLong=" + dest_in[1]
    else:
        dest_string = "destId=" + dest_in

    trip_key = '6828b3e5b4b04737b4311891e825e913'

    get_trip = "http://api.sl.se/api2/TravelplannerV3_1/trip.json?key=" + trip_key + "&" + origin_string + "&" + dest_string + "&Date=" + d1 + "&Time=" + dest_time + "&searchForArrival=1"
